...
from leads.ivr import ivr_user
...
#     # if request.is_ajax():
#     #     session_data = request.session.get('leaddata', {})
#     #     print(request.session.items())

#     # #     session_data = request.session.get('leaddata', {})
        
#     #     searchText = session_data.get('searchText')
...
#     #     creation_date_sort = session_data.get('creation_date_sort')
#     #     filter_created_date = session_data.get('filter_created_date')
#     #     filter_updated_dates = session_data.get('filter_updated_dates')
#     #     filter_date_range = session_data.get('filter_date_range')
#     #     filter_update_range = session_data.get('filter_update_range')
#     #     locationq = session_data.get('locationq')
#     #     star_sort = session_data.get('star_sort')
#     #     telecaller = session_data.get('telecaller')
#     #     offsetval = session_data.get('offsetval')
...
#     #             Q(fbform_id__icontains=searchText) |
...
#     #     if type:
#     #         leads = leads.filter(type=type)

#     #     if product:
#     #         leads = leads.filter(product=product)
#     #     # if product:
#     #     #     leads = leads.filter(product__name__icontains=product)
...
#     #     if filter_created_date:
#     #         leads = leads.filter(created_at__range=[datetime.datetime.strptime(filter_created_date, '%Y-%m-%d'), datetime.datetime.strptime(filter_created_date, '%Y-%m-%d') + datetime.timedelta(days=1)])

#     #     if filter_updated_dates:
#     #         filter_updated_dates_list = filter_updated_dates.split(',')
#     #         leads = leads.filter(updated_at__range=[datetime.datetime.strptime(filter_updated_dates_list[0], '%Y-%m-%d'), datetime.datetime.strptime(filter_updated_dates_list[1], '%Y-%m-%d') + datetime.timedelta(days=1)])

#     #     if filter_date_range:
#     #         filter_date_range_list = filter_date_range.split(',')
#     #         leads = leads.filter(created_at__range=[datetime.datetime.strptime(filter_date_range_list[0], '%Y-%m-%d'), datetime.datetime.strptime(filter_date_range_list[1], '%Y-%m-%d') + datetime.timedelta(days=1)])

#     #     if filter_update_range:
#     #         filter_update_range_list = filter_update_range.split(',')
#     #         leads = leads.filter(updated_at__range=[datetime.datetime.strptime(filter_update_range_list[0], '%Y-%m-%d'), datetime.datetime.strptime(filter_update_range_list[1], '%Y-%m-%d') + datetime.timedelta(days=1)])
...
#     #     if star_sort:
#     #         if star_sort == 'asc':
#     #             leads = leads.order_by('star')
#     #         else:
#     #             leads = leads.order_by('-star')

#     #     if telecaller:
#     #         leads = leads.filter(telecaller_id=telecaller)
...
#     #     for lead in leads:
#     #             lead_item = {
#     #                 'leadid': f'<span class="taglabel"><i class="fa fa-exclamation-circle" title="Transferred by {get_transferred_user(lead.id)} to {get_transferredto_user(lead.id)}" aria-hidden="true"></i></span>{lead.id}' if lead.is_transfer == 1 else lead.id,
#     #                 'date': lead.created_at.strftime('%Y-%m-%d %H:%M:%S'),
#     #                 'city': lead.city.capitalize(),
#     #                 'name': lead.name.capitalize(),
#     #                 # 'product': get_product_by_id(lead.product_id).name if lead.product_id else '',
#     #                 'product': get_product_name_by_id(lead.product_id),

#     #                 'updated_date': lead.updated_at.strftime('%Y-%m-%d %H:%M:%S'),
#     #                 'source': f'{get_source_by_id(lead.lead_source_id).name}<a href="#" data-toggle="tooltip" title="{get_source_user(lead.lead_source_id)}"><i class="fa fa-info-circle"></i></a>' if lead.lead_source_id else '',
#     #                 'stage': f'{get_status_by_id(lead.lead_status_id).name} on {lead.ringing_date.strftime("%Y-%m-%d")}' if lead.lead_status_id == '23' else get_status_by_id(lead.lead_status_id).name if lead.lead_status_id else '',
#     #                 'keyword': lead.gkeyword if lead.gkeyword else '',
#     #                 'actions': f'<a onclick="ivrcall({lead.phone},{lead.id})" style="color:red" href="javascript:void(0);" class="clicktocall" title="click2call"><i class="fa fa-volume-control-phone"></i></a>'
#     #                             f'<a href="{lead.id}/edit" title="Edit"><i class="fas fa-edit"></i></a>'
#     #                             f'<a onclick="return confirm(`Do you really want to delete this lead?`)" style="color:red" href="{lead.id}/delete" title="Delete"><i class="fa fa-trash"></i></a>'
#     #                             f'<a href="#" data-lead_id="{lead.id}" data-toggle="modal" data-target="#lead_assigning_modal" title="Add lead transfer" class="openlead_assigning_modal"><i class="fas fa-user-plus"></i></a>' if lead.is_transfer != 1 else ''
#     #                             f'<a href="#" data-lead_id="{lead.id}" data-toggle="modal" data-target="#attachmentModal" title="Add attachment" class="openAttachmentModal"><i class="fas fa-list"></i></a>',
#     #                 # ... additional column data ...
#     #             }
#     #             lead_data.append(lead_item)
...
def leadlisting(request):
    ...
    ivr_users_data = []
    if user_department == 'telecaller':
        ivr_users_data.append({'name': user_data.username, 'user_id': user_data.user_id})
    else:
    ...
    # Your existing code here
        if request.is_ajax():
            ...
            if request.GET.get('filter_update_range'):
                ...
                start_update = datetime.strptime(start_update.strip(), "%Y-%m-%d")
                end_update = datetime.strptime(end_update.strip(), "%Y-%m-%d")
            ...
            for row in leads[start:start+length]:
                ...
                is_selected = str(row.id) in selected_lead_ids
...
def telecaller_leads(request):
    ...
    try:
        ...
        if request.is_ajax():
            ...
            if request.GET.get('filter_updated_dates'):
                updated_dates_list = request.GET.get('filter_updated_dates').split(',')
            ...
            if request.GET.get('transfer_user'):
                leads = leads.filter(assigned_to=request.GET.get('transfer_user'))
...
#     # telecaller_users = User.objects.filter(department_id=1).exclude(id=user_data.id).values('id', 'name')
#     # telecallers_id = userdetail.objects.filter(uniqueid=clientid, department='telecaller').values_list('user_id', flat=True)
#     # telecaller_users = User.objects.filter(id__in=telecallers_id)
#     # print(telecaller_users)
#     # user_ivr_number = userData.ivr_virtual_number

#     # ivruser = []
#     # if usreDepartment == 1:
#     #     ivruser.append({'name': userData.name, 'user_id': userData.ivr_user_id})
#     # else:
#     #     ivrcontrol = IvrController()
#     #     ivruser = ivrcontrol.ivr_user()
...
#     # else:
#     #     pagerest = "false"
#     # leads = Leads.objects.filter(~Q(lead_status_id=25))
...
#         #     'filter_date_range': request.GET.get('filter_datee_range'),
#         #     'filter_updated_dates': request.GET.get('filter_updated_dates'),
#         #     'transfer_user': request.GET.get('transfer_user'),
#         #     'filter_telecaller': request.GET.get('filter_telecaller'),

#         #     # 'attachment_status': request.GET.get('attachment_status')
#         # }
#         locationq = request.GET.get('locationq')
#         star_sort = request.GET.get('star_sort')
#         unreadmsg = request.GET.get('unreadmsg')

#         # if leaddata['searchText']:
#         #     leads = leads.filter(
#         #         Q(name__icontains=leaddata['searchText']) |
#         #         Q(phone__icontains=leaddata['searchText']) |
#         #         Q(email__icontains=leaddata['searchText']) |
#         #         Q(fbform_id__icontains=leaddata['searchText']) |
#         #         Q(id__icontains=leaddata['searchText'])
#         #     )

#         # if leaddata['status']:
#         #     leads = leads.filter(lead_status_id=leaddata['status'])

#         # if leaddata['type']:
#         #     leads = leads.filter(lead_type_id=leaddata['type'])

#         # if leaddata['product']:
#         #     leads = leads.filter(product_id=leaddata['product'])

#         # if leaddata['source']:
#         #     leads = leads.filter(lead_source_id=leaddata['source'])

#         # # Handle date filters
#         # if leaddata['filter_created_date']:
#         #     created_date_list = leaddata['filter_created_date'].split(',')
#         #     created_date_list = [date for date in created_date_list]
#         #     leads = leads.filter(created_at__date__in=created_date_list)

#         # if leaddata['filter_updated_dates']:
#         #     updated_dates_list = leaddata['filter_updated_dates'].split(',')
#         #     updated_dates_list = [date for date in updated_dates_list]
#         #     leads = leads.filter(updated_at__date__in=updated_dates_list)

#         # if leaddata['transfer_user']:
#         #     leads = leads.filter(assigned_to=leaddata['transfer_user'])

#         # if leaddata['filter_telecaller']:
#         #     leads = leads.filter(assigned_to_id=leaddata['filter_telecaller'])

#         # if leaddata['creation_date_sort'] == 'asc':
#         #     leads = leads.order_by('created_at')
#         # elif leaddata['creation_date_sort'] == 'desc':
#         #     leads = leads.order_by('-created_at')
...
#         # if request.GET.get('filter_created_date'):
#         #     created_date_range = request.GET.get('filter_created_date').split('-')
#         #     start_date = datetime.strptime(created_date_range[0].strip(), "%m/%d/%Y").strftime("%Y-%m-%d")
#         #     end_date = datetime.strptime(created_date_range[1].strip(), "%m/%d/%Y").strftime("%Y-%m-%d")
#         #     leads = leads.filter(created_at__range=[start_date, end_date])

#         # if request.GET.get('filter_updated_dates'):
#         #     updated_date_range = request.GET.get('filter_updated_dates').split('-')
#         #     start_date = datetime.strptime(updated_date_range[0].strip(), "%m/%d/%Y").strftime("%Y-%m-%d")
#         #     end_date = datetime.strptime(updated_date_range[1].strip(), "%m/%d/%Y").strftime("%Y-%m-%d")
#         #     leads = leads.filter(updated_at__range=[start_date, end_date])
#         #     print(updated_date_range, start_date, end_date, '***********************')
...
#         #     updated_dates_list = request.GET.get('filter_updated_dates').split(',')
#         #     updated_dates_list = [datetime.strptime(date, "%Y-%m-%d") for date in updated_dates_list]
#         #     leads = leads.filter(updated_at__in=[datetime.combine(date, datetime.min.time()) for date in updated_dates_list])
#         #     # leads = leads.filter(updated_at__in=updated_dates_list) 
#         #     print(leads,'filter_updated_dates')
#         #     print(updated_dates_list, request.GET.get('filter_updated_dates'))   

#         # if request.GET.get('filter_updated_dates'):
#         #     updated_dates_list = request.GET.get('filter_updated_dates').split(',')
#         #     updated_dates_list = [datetime.strptime(date.strip(), "%Y-%m-%d") for date in updated_dates_list]
            
#         #     # Convert naive datetime objects to time zone-aware datetime objects
#         #     updated_dates_list = [timezone.make_aware(date, timezone.get_default_timezone()) for date in updated_dates_list]

#         #     # Create a date range for filtering within the selected day
#         #     start_time = timezone.datetime.combine(updated_dates_list[0], timezone.datetime.min.time())
#         #     end_time = timezone.datetime.combine(updated_dates_list[0], timezone.datetime.max.time())
            
#         #     leads = leads.filter(updated_at__range=(start_time, end_time))
...
#         # if request.GET.get('filter_created_date'):
#         #     created_date_list = request.GET.get('filter_created_date').split(',')
#         #     created_date_list = [datetime.strptime(date.strip(), "%Y-%m-%d") for date in created_date_list]
            
#         #     # Convert naive datetime objects to time zone-aware datetime objects
#         #     created_date_list = [timezone.make_aware(date, timezone.get_default_timezone()) for date in created_date_list]
            
#         #     # If created_at field includes time, provide a time range for filtering
#         #     start_time = timezone.datetime.combine(created_date_list[0], timezone.datetime.min.time())
#         #     end_time = timezone.datetime.combine(created_date_list[0], timezone.datetime.max.time())
            
#         #     leads = leads.filter(created_at__range=(start_time, end_time))
            
#         #     print(leads, 'filter_created_date')
#         #     print(created_date_list, request.GET.get('filter_created_date'))
...
#         # if unreadmsg:
#         #     query = Messages.objects.filter(chat_read=0, sender_role='user').values('phone')
#         #     unreaddata = [f'+91{value["phone"]}' if len(value["phone"]) == 10 else value["phone"] for value in query]
#         #     instance = instance.filter(phone__in=unreaddata)
...
#             leadid = ''
#             if lead.is_transfer == 1 and lead.transfer_to != user_id:
#                 # actions += f'<span>Transferred to {get_source_by_id(lead.id)}</span>'
#                 actions += f'<span>Transferred to {get_source_by_id(lead.id)}</span>'
...
#                 if user_department == 'telecaller':
#                     actions += f' <a onclick="openlead_transfermodal({lead.id})" class ="openlead_transfer_modal" title="Add lead transfer"><i class="fas fa-exchange-alt"></i></a>'
...
#     # print(telecaller_users)
#     if request.is_ajax():
#         lead_data = request.POST

#         request.session['leaddata.searchText'] = lead_data.get('searchText')
#         request.session['leaddata.status'] = lead_data.get('status')
#         request.session['leaddata.type'] = lead_data.get('type')
#         request.session['leaddata.product'] = lead_data.get('product')
#         request.session['leaddata.source'] = lead_data.get('source')
#         # ... (other session data)

#         leads = Leads.objects.all()  # Fetch your leads queryset

#         filtered_leads = leads.filter(
#             Q(name__icontains=lead_data.get('searchText')) |
#             Q(id__icontains=lead_data.get('searchText')) |
#             Q(email__icontains=lead_data.get('searchText')) |
#             Q(fbform_id__icontains=lead_data.get('searchText')) |
#             Q(phone__icontains=lead_data.get('searchText'))
#         )

#         if lead_data.get('status'):
#             filtered_leads = filtered_leads.filter(lead_status_id=lead_data.get('status'))

#         if lead_data.get('type'):
#             filtered_leads = filtered_leads.filter(lead_type_id=lead_data.get('type'))

#         if lead_data.get('product'):
#             filtered_leads = filtered_leads.filter(product_id=lead_data.get('product'))

#         if lead_data.get('source'):
#             filtered_leads = filtered_leads.filter(lead_source_id=lead_data.get('source'))
...
#             actions = ''
#             if lead.is_transfer == 1 and lead.transfer_to != user_id:
#                 actions += ' <span>Transferred to {get_source_by_id(lead.id)}</span>'
#             else:
#                 actions += ' <a href="javascript:void(0);" onclick="ivrcall({lead.phone},{lead.id})" style="color:red" class="clicktocall" title="click2call"><i class="fa fa-volume-control-phone"></i></a>'
#                 # actions += f' <a href="{url}" title="Edit"><i class="fas fa-edit"></i></a>'.format(url=reverse('leads.edit', kwargs={'id': lead.id}))
#                 actions += ' <a href="{url}" title="Edit"><i class="fas fa-edit"></i></a>'.format(url=reverse('leads.edit', kwargs={'id': lead.id}))
...
#                 if user_department == 'telecaller':
#                     actions += ' <a href="#" onclick="openlead_transfermodal({lead.id})" title="Add lead transfer"><i class="fas fa-exchange-alt"></i></a>'
...
#             'data': lead_list,
#             'draw': 1,  # Draw count for DataTables
#             'recordsTotal': filtered_leads.count(),
#             'recordsFiltered': filtered_leads.count(),
#         }
...
#             'pagerest': pagerest,
#         }
#         return render(request, 'leads/telecaller-index.html', context)
...
def lead_export(request):
    telecaller_id = None
    userid = userdetail.objects.get(user_id=request.user.id)
    # client_id = udetail.uniqueid
    # user_department = udetail.department
    user_department = userid.department
    user_data = request.user
    # user_department = user_data.userdetail.department_id
                    ...
                    if phone:
                        ...
                        if not duplicate_lead:
                            ...
                            lead_data = {
                                ...
                                "phone": phone,
                                "email": email,
                                "product_id": product if product else None,  # Change here
                                "lead_type_id": lead_type if lead_type else None,  # Change here
                                "lead_source_id": source if source else None,  # Change here
                                "lead_status_id": lead_status if lead_status else None,  # Change here
                                "created_at": timezone.now(),
                                "updated_at": timezone.now(),
                                "created_by": "system" if udetail else None,  # Change here
                                "is_potential": "no",
                                "centre_name": city if city else None,  # Change here
                                "star_patient": "no",
                                "client_id": clientId,
                            }
...
def update_lead_assigning(request):
    ...
    try:
        ...
        return JsonResponse({"message": "Leads assigned successfully.", "success": 1})
